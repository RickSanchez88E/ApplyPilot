...
本項目常用測試命令範例：
- TypeScript 類型檢查：`npx tsc --noEmit`
- 單元測試：`npx vitest run`
- 前端 lint：`cd frontend && npm run lint`
- 前端構建：`cd frontend && npm run build`
- 組合檢查：`npx tsc --noEmit && npx vitest run`
...
```
用戶: /for-loop "修復所有 TypeScript 錯誤" --max-iterations 5 --test-command "npx tsc --noEmit"
...
腳本: CONTINUE: 測試失敗 (迭代 1/5)
      # 錯誤: src/foo.ts(12,5): error TS2322
