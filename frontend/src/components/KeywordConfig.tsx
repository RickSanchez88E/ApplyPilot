...
export function KeywordConfig({ onConfigChange }: KeywordConfigProps) {
  ...
  const handleSave = async () => {
    if (keywords.length === 0) {
      setMessage({ text: '至少需要一个关键词', type: 'error' });
      return;
    }
    setSaving(true);
    setMessage(null);
    ...
    <div className="panel p-5">
      ...
      <div className="space-y-2 mb-4">
        ...
        <div className="flex gap-1.5">
          <input
            ...
            placeholder="添加关键词..."
            className="flex-1 px-3 py-1.5 rounded-lg border border-[var(--color-border)] bg-white text-sm font-mono focus:outline-none focus:border-[var(--color-accent)] transition-colors"
          />
          <button
            onClick={addKeyword}
      ...
      <div className="space-y-2 mb-4">
        ...
        <input
          ...
          className="w-full px-3 py-1.5 rounded-lg border border-[var(--color-border)] bg-white text-sm font-mono focus:outline-none focus:border-[var(--color-accent)] transition-colors"
        />
      </div>
